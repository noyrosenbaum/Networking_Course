...
def server(host: str, port: int) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        ...
        # Prepare the server socket - COMPLETED BY SUBMITTERS (NOY AND YAEL)
        ...
        while True:
            try:
                # Establish connection with client. - COMPLETED BY SUBMITTERS (NOY AND YAEL)
...
def client_handler(client_socket: socket.socket, client_address: tuple[str, int]) -> None:
    ...
    with client_socket:  # closes the socket when the block is exited
        ...
        while True:
            # Receive data from the client - COMPLETED BY SUBMITTERS (NOY AND YAEL)
            ...
            try:
                ...
                # Send the response back to the client - COMPLETED BY SUBMITTERS (NOY AND YAEL)
